...
echo '<div ' . $this->get_render_attribute_string( 'wrapper' ) . '>';
	...
	if ( $products ) { ?>
		<div class="<?php echo $class ?>" data-first_tab_id="<?php echo $first_tab_id ?>">
			<?php
			foreach ($products as $product) {
				...
				?>
					<div class="anant-single-grid-product <?php echo 'anant-tabs-'.$this->get_id() ?>
						<?php
							if ( ! $thumbnail_id ) {
								echo 'anant-no-grid-product-image';}
							?>"
							...
							if ( !empty( $terms ) ) {
								foreach ( $terms as $term ) {
									if ( array_key_exists($term->term_id, $swiper_div_tabs)){
										echo $swiper_div_tabs[$term->term_id];
									}
								}
							}
						}
